...
<head>
    ...
    <script>
        const reader_writer = @json($reader_writer);
        const readerSubPlan = @json($readerSubPlan);
        const writerSubPlan = @json($writerSubPlan);
        const books = {
            category: @json($books['categories']),
            data: @json($books['items'])
        }
    </script>
...
<body class="overflow-x-hidden">
    ...
    <main class="row" style="height: 95vh !important">
        ...
        <section class="col-10 row g-4 p-5">
            <div class="row g-3">
                <div class="col-6 p-custom">
                    <div>
                        <canvas id="liseur"></canvas>
                    </div>
                </div>
                <div class="col-6 p-custom">
                    <div>
                        <canvas id="ecrivain"></canvas>
                    </div>
                </div>
                <div class="col-6 p-custom">
                    <div>
                        <canvas id="livres"></canvas>
                    </div>
                </div>
                <div class="col-6 p-custom">
                    <div>
                        <canvas id="liseur-ecrivain"></canvas>
                    </div>
                </div>
            </div>
        </section>
